...
function BookFiltersForm({ idPrefix, initialParams }: { idPrefix: string; initialParams: SearchParams }) {
  ...
  function commit(patch: Partial<SearchParams>) {
    ...
    startTransition(() => {
      ...
      void router.replace(hrefToTarget(buildHref(next)), { scroll: false });
    });
  }
        ...
        <div className="border-divider dark:border-divider-dark border-t pt-3">
          <Button
            ...
            onClick={() =>
              startTransition(() => {
                dispatch({ type: 'reset' });
                void router.replace(hrefToTarget(buildHref({ delay: filters.delay })), { scroll: false });
              })
            }
            variant="secondary"
          >
...
export function BookFilters({ idPrefix }: { idPrefix: string }) {
  ...
  return <BookFiltersForm idPrefix={idPrefix} initialParams={parseSearchParams(searchParams)} />;
}
