...
const DATA_FILE = path.resolve(process.env.AUTHORS_DATA_PATH ?? './src/lib/db/authors.json');
...
async function batchInsertAuthors(batch: AuthorData[], sqlQuery: NeonQueryFunction<false, false>) {
  const insertQuery = `
